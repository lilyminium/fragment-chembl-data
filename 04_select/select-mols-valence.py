...
def select_by_parameter_diversity(
    ...
    between atoms in the parameter.
    ...
    parameter_types = ["Bonds", "Angles", "ImproperTorsions"]
    for i, smi in enumerate(smiles):
        ...
        for parameter_type in parameter_types:
            for ix, value in labels[parameter_type].items():
                if value.id == parameter_id:
                    all_involved_indices |= set(ix)

        other_parameter_ids = set()
        for parameter_type in parameter_types:
            for ix, value in labels[parameter_type].items():
                if set(ix) & all_involved_indices:
                    other_parameter_ids.add(value.id)
